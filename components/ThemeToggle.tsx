...
export const ThemeToggle: React.FC<ThemeToggleProps> = ({ isDark, onToggle }) => {
  return (
    <button
      onClick={onToggle}
      className="relative w-14 h-7 rounded-full transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
      style={{
    ...
    >
      ...
      <span className="absolute inset-0 flex items-center justify-between px-1.5 pointer-events-none">
        ...
        <svg
          ...
          style={{ 
            color: isDark ? 'rgba(255,255,255,0.5)' : 'transparent',
            opacity: isDark ? 1 : 0 
          }}
          fill="currentColor"
          viewBox="0 0 20 20"
        ...
        <svg
          className="w-4 h-4 transition-opacity duration-200"
          style={{ 
            color: !isDark ? 'var(--text-tertiary)' : 'transparent',
            opacity: !isDark ? 1 : 0 
          }}
          fill="currentColor"
          viewBox="0 0 20 20"
